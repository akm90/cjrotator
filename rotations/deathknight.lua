...
local function CJCheckFrostBuffs()
	...
	if CJCooldown("Horn of Winter") == 0 and (not CJ_HasBuff("player","Horn of Winter") and not (CJ_HasBuff("player","Battle Shout") or CJ_HasBuff("player","Strength of Earth Totem"))) then
...
function CJFrostDKRot()
	...
	if select(2,CJ_DebuffInfo("target","Frost Fever")) < 2 and CJCooldown("Howling Blast") == 0  then
