...
function CJArmsWarRot()
	...
	if GetShapeShiftForm() ~= 1 and CJCooldown("Battle Stance") == 0 and((select(2,CJ_DebuffInfo("target","Rend"))) or (CJ_HasBuff("player","Taste for Blood") and CJCooldown("Mortal Strike") > 1 and UnitPower("player") <= 75) then
		CastShapeshiftForm(1);
		return;
